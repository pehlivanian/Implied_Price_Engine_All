...
int main(int argc, char **argv)
{
    ...
    int r3 = pool.submit(bid_p( SecPair(1, 3, 1), QUOTE(-200, 1), IE)).get();

    int r4 = pool.submit(bid_p( SecPair(0, 2, 1), QUOTE(-200, 1), IE)).get();
    int r5  =pool.submit(bid_p( SecPair(2, 3,  1), QUOTE(-100, 1), IE)).get();
    int r6 = pool.submit(bid_p( SecPair(3, -1, 1), QUOTE(4805, 1), IE)).get();
